...
import com.example.santiagolopezgarcia.talleres.presenters.MapaTareasPresenter;
...
public class MapaTareas extends BaseActivity<MapaTareasPresenter> implements OnMapReadyCallback, BaseView {

    List<TareaXOrdenTrabajo> listaTareaXOrdenTrabajo;
    ...
    private void iniciarActivity() {
        configurarActionBar();
        SupportMapFragment mapOrdenesTrabajo = (SupportMapFragment)
                getSupportFragmentManager().findFragmentById(R.id.mapOrdenesTrabajo);
    ...
    public void mostrarOrdenesTrabajo() {
        Thread thread = new Thread(() -> {
            try {
                listaTareaXOrdenTrabajo = DatosCache.getListaTareaXOrdenTrabajoFiltrado();
                if (listaTareaXOrdenTrabajo.isEmpty()) {
                    listaTareaXOrdenTrabajo = DatosCache.getListaTareaXOrdenTrabajo();
                }
                ...
                listaOrdenesTrabajo = presentador.obtenerListaOrdenes(listaTareaXOrdenTrabajo);
                ...
                final LatLng finalGpsPrimeraOrdenTrabajo = gpsPrimeraOrdenTrabajo != null ? gpsPrimeraOrdenTrabajo :
                        new LatLng(gps.getLocation().getLatitude(),
                                gps.getLocation().getLongitude());
                runOnUiThread(() -> {
                    googleMap.animateCamera(CameraUpdateFactory.newLatLngZoom(finalGpsPrimeraOrdenTrabajo, 15));
                });
    ...
    private void getRouteByOriginDestin(LatLng origin, LatLng destination) {
        ...
        HttpGet del = new HttpGet(
                "http://maps.googleapis.com/maps/api/directions/json?origin=" + origin.latitude +
                        ",%20" + origin.longitude + "&destination=" + destination.latitude + ",%20"
